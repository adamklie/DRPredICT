...
class MultiomicDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None) -> None:
        ...
        if self.subset:
            ...
            self.train, self.test = random_split(dataset, [train_len, test_len], generator=torch.Generator().manual_seed(13))
            val_len = int(dataset_len*0.2)
            train_len = train_len - val_len
            self.train, self.val = random_split(self.train, [train_len, val_len], generator=torch.Generator().manual_seed(13))
